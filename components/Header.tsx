        ...
        <div className='border-b top-0 w-full'>
            <nav className='border-gray-200 px-4 lg:px-6 md:px-6 py-2.5 dark:bg-white'>
                <div className='flex flex-wrap justify-between items-center mx-auto max-w-screen-xl'>
                    ...
                    <div className={`absolute top-16 justify-between bg-white w-1/3 z-50 right-0 mr-4 rounded-lg border lg:items-center md:items-center lg:flex lg:w-auto md:flex md:w-auto lg:order-1 md:order-1 md:border-none lg:border-none m-2 shadow-black lg:relative md:relative lg:top-0 md:top-0 ${!isOpen && "hidden"}`} id="mobile-menu-2">
                        <ul className={`flex flex-col lg:flex-row lg:space-x-8 md:flex-row md:space-x-8 shadow-black font-medium`}>
                            {Menus.map((menu, i) => (
                                <li key={i}>
                                    <Link href={menu.to} className="block py-2 pl-3 border transition hover:border-indigo-500  hover:text-indigo-500 border-100 lg:border-0 md:border-0 lg:p-0 md:p-0">{menu.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </div>
